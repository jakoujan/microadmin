...
ALTER TABLE PRODUCT_ORDER ADD COLUMN `status` INTEGER NOT NULL;
...
CREATE FORCE VIEW PRODUCTS_PREPARATION AS
SELECT PO.ID, PO.QUANTITY, P.DESCRIPTION, TC.NAME AS TABLE_NAME, PO.COMMENT, PO.STATUS, P.SECTION AS "SECTION" FROM PRODUCT_ORDER PO INNER JOIN PRODUCT P ON P.ID = PO.PRODUCT INNER JOIN ORDER_COMAND OC ON OC.ID = PO.ORDER_COMAND
INNER JOIN TABLE_CHAIR TC ON OC.TABLE_CHAIR = TC.ID;
...
INSERT INTO `TAX_TYPE` (TAX_TYPE, TAX_TYPE_DESCRIPTION, ACTIVE, PERCENTAGE) 

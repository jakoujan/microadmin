...
SELECT OS.ID as ID, S.ID AS SALE, S.SALE_DATE, S.TOTAL_AMOUNT, S.QUANTITY, OS.ORDER_COMAND AS ORDER_COMAND, OC.CASHIER AS CASHIER FROM SALE S INNER JOIN ORDER_SALE OS ON (OS.SALE = S.ID) LEFT JOIN ORDER_COMAND OC ON (OS.ORDER_COMAND = OC.ID);

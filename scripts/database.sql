...
SELECT O.ID AS ID, TC.`NAME` AS `TABLE_NAME`, RESPONSIBLE, TOTAL_AMOUNT, O.STATUS, SS.`NAME` AS ORDER_STATUS_NAME FROM ORDER_COMAND O INNER JOIN TABLE_CHAIR TC ON (TC.ID = O.TABLE_CHAIR) INNER JOIN SALE_STATUS SS ON(SS.ID = O.STATUS);

CREATE FORCE VIEW SALES_REPORT_VIEW AS
SELECT OS.ID as ID, S.ID AS SALE, S.SALE_DATE, S.TOTAL_AMOUNT, S.QUANTITY, OS.ORDER_COMAND AS ORDER_COMAND, OC.CASHIER AS CASHIER, C.NAME as CASHIER_NAME FROM SALE S INNER JOIN ORDER_SALE OS ON (OS.SALE = S.ID) LEFT JOIN ORDER_COMAND OC ON (OS.ORDER_COMAND = OC.ID) LEFT JOIN USER C ON (C.ID = OC.CASHIER);
